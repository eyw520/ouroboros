...
- **Seed from inside a target repo** → the `seed` skill (`/seed`); locates this checkout through its user-level symlink and runs adopt against the cwd.
...
- Commit messages: `<type>: Sentence case ending with period.`, type ∈ feat|fix|docs|chore|clean|revert (enforced by `.githooks/commit-msg`; scopeless, body-rejecting, cosmetic-auto-fixing).
